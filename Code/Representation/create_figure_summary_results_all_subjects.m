function [fh, Y] = create_figure_summary_results_all_subjects(...
    iSess,iRecon)
...
%               figure
% EXAMPLE
%   create_figure_summary_results_all_subjects(1, 1)
%
%   See also
%
% Author:   Lars Kasper
...
for iSubj = 1:nSubjects
    ...
    axis off;
    set(gcf, 'Name', sprintf('%s %s: %d %s', details.subjectId, ...
        details.recon, nImages+1, nameArray{nImages+1}));
    ...
    axis off;
    set(gcf, 'Name', sprintf('%s %s: %d %s', details.subjectId, ...
        details.recon, nImages+2, nameArray{nImages+2}));
end
