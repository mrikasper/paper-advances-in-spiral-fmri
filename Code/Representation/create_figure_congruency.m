...
for f = 1:nFiles-1
    ...
    if figOptions.doCreateEdgeSeparately
        ...
        Z.data(:) = 0;
        fh(f) = Z.threshold(displayRangeFunctional).plot_overlays(...
            mean(Y{nFiles}.threshold(figOptions.thresholdEdgeOutliers), iDimEcho), 'selectedSlices', figOptions.selectedSlice, 'overlayMode',  'edge', ...
        ...
        fh(f) = Y{f}.select('t',1).threshold(displayRangeFunctional).plot_overlays(...
            mean(Y{nFiles}.threshold(figOptions.thresholdEdgeOutliers), iDimEcho), 'selectedSlices', figOptions.selectedSlice, 'overlayMode',  'edge', ...
            'edgeThreshold', figOptions.thresholdEdge, ...
            'colorBar', 'off', sharedParameters{:});
